...
func _ready() -> void:
	...
	if levels.is_empty():
		push_warning("Offline game has no levels configured.")
		return
...
func load_level(index: int) -> void:
	if index < 0 or index >= levels.size():
		push_warning("Invalid offline level index: %d" % index)
		return
...
func _on_player_died() -> void:
	if _game_complete or _current_level_index < 0:
		return
...
func _update_level_label(game_complete: bool) -> void:
	if game_complete:
		level_label.text = "All offline levels cleared!"
		return

	level_label.text = "Offline Level %d / %d" % [_current_level_index + 1, levels.size()]


func _show_completion_screen() -> void:
	if completion_screen == null or not completion_screen.has_method("open"):
		return

	completion_screen.open(
		"Level Clear!",
